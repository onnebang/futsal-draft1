...
window.FD = {
  ...
  ERROR_MESSAGE: {
    ...
    ORDER_NOT_LOCKED: '아직 지명 순서가 확정되지 않았어요.',
    NOT_TOP_TEAM: '매치 순서는 투표 1위 팀 코치만 정할 수 있어요.',
    BAD_ORDER: '경기 순서가 올바르지 않아요. 새로고침 후 다시 시도해주세요.',
  ...
  // 총 23명(참석 20 + 불참 3) 을 끊지 않고 한 번에 스네이크로 드래프트한다 —
  // 7라운드(3픽) + 마지막 2픽, 라운드마다 방향이 뒤집힌다. 불참 여부는 지명을
  // 막지 않고, 코치 화면에서 "불참자" 라벨로만 참고된다(draft_players.attending).
  TOTAL_PICKS: 23,
};
