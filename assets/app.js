...
(function () {
  ...
  const demo = {
    ...
    // 참석 20명 + 불참 3명(송희·승민·인선) = 총 23명. 불참자도 투표·각오·캐릭터는
    // 그대로 하고, 드래프트도 참석 여부와 무관하게 스네이크 순번 그대로 아무 때나
    // 뽑을 수 있다 (드래프트 자체를 막지 않음 — attending은 코치 화면 라벨용일 뿐).
    ...
    // 코치 보드 리허설용 상태 — 데모용으로 참석자 예시를 먼저, 불참자 예시를 뒤에
    // 배치했을 뿐, 실제로는 참석 여부와 무관하게 어떤 순서로 뽑아도 된다.
    coachState() {
