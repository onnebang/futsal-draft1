...
- **REVEAL 발표가 다 끝나기 전엔 순서를 보여주지 않는다.** `draft_order` 는
  ...
  전원에게 이미 공개된 정보이므로 무조건 연다.
- 최종 결과는 코치 픽과 똑같은 `{ 이름: 팀코드 }` 한 장으로만 서버에 저장한다.
  화면에서 순번대로 고르는 건 사용자 경험일 뿐, `submit_mock_draft` 는 이 순번을
  받지도 저장하지도 않는다(23명이 다 채워졌는지, 값이 실존 팀 코드인지만 본다) —
  픽 순서 자체가 서버에 남지 않으니 요구사항 1을 어길 방법이 애초에 없다.
- **순번이 아니라 팀 배정만 묻는다.** "몇 번째로 뽑힐지"는 절대 묻지 않는다 —
  요구사항 1(개인 지명 순번 비공개)을 이 기능에서도 그대로 지키기 위한 설계다.
  채점도 `draft_rosters.members`(이미 가나다순으로 공개되는, 순번이 없는 최종 명단)와
  대조할 뿐, `draft_picks.seq` 는 어디에서도 읽지 않는다.
  이 랭킹은 **참가자(맞힌 사람)** 를 줄 세우는 것이지 선수를 평가하는 게 아니라서
  요구사항 2(개인 평점·랭킹 금지)와도 무관하다 — PREDICT 단계의 우승 예측 적중자
  랭킹과 같은 종류다.
- `draft_mock_drafts(player_name pk, assignments jsonb, updated_at)` — SELECT 정책은
  열려 있다(누구나 서로의 예측을 볼 수 있음, 명단 공개가 의도인 테이블들과 동일 패턴).
  쓰기는 `submit_mock_draft` 로만 가능하고, **23명 전원이 채워지고 값이 전부 실존
  팀 코드일 때만** 통과시킨다 — 일부만 채운 예측은 채점 공정성이 깨지므로 부분
  제출을 허용하지 않는다(다른 `submit_*` 함수들의 "조용히 버림" 패턴과 다르다).
  `player_name` 이 PK라 `on conflict do update` 로 **언제든 다시 제출해 고칠 수 있다**
  (투표처럼 한 번뿐인 게 아니라 각오/캐릭터처럼 계속 고칠 수 있는 쪽을 택했다 —
  드래프트 창이 며칠씩 열려 있어서, 생각이 바뀌면 바로 반영되는 게 더 재밌다).
  `REVEAL`·`DRAFT` 단계에서만 제출 가능 — 결과가 공개되면 잠긴다.
- `mock_draft_leaderboard()` — `draft_rosters` 를 펼쳐 만든 (팀, 선수) 답안표와
  각자의 `assignments` 를 대조해 정답 수를 센다. `SECURITY DEFINER` 가 아닌
  일반 `stable sql` 함수다(`vote_tally`/`standings` 와 같은 스타일) — 쓰기가 없고
  이미 공개된 테이블만 읽어서 권한 상승이 필요 없다. `draft_rosters` 가 비어 있으면
  (RESULT 공개 전) 전원 0점으로 나올 뿐이라 별도 예외 처리를 하지 않는다.
- 참여 신원은 무기명이 아니라 **본인이 고른 실명**이다(`draft_players.name` 그대로) —
  투표와 달리 팀 배정을 맞히는 건 민감 정보가 아니라서, 수정 가능한 upsert 키로도
  쓸 수 있는 실명 쪽이 예측/댓글의 익명 닉네임보다 이 기능엔 더 맞다.
  이름을 아직 안 고른 사람이 참여하면(`#내캐릭터` 와 같은 패턴) 이름 선택 그리드를
  먼저 보여주고, **제출에 성공한 시점에만** `store.set('name', ...)` 으로 전역
  신원을 확정한다 — 고르기만 하고 제출은 안 했다면 아직 신원이 묶이지 않는다.
